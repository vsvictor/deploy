...
class ModifyAgreement  : ProtectedHttpEndpoint() {
    override val name = "modify_agreement"
    ...
    override fun implementSafeEndpoint(socketAddress: SocketAddress, request: HttpRequest): HttpResponse {
        ...
        val agr = Agreement(userId, all_points, points, started, confirmed)
        try {
            //Agreement.mapper.saveItem(agr)
            var e = Agreement.mapper.items.find { it -> it.userId.equals(userId) }
            e.started = started
            e.all_point = all_points
            e.points = points
            e.confirmed = confirmed
            Agreement.mapper.saveItem(e)
